...
int main()
{
	...
	VideoCapture video("challenge_video.mp4"); // 영상 불러오기
	...
	double fps = 25.0; // 프레임
	string filename = "./result.avi"; // 결과파일
	...
	while (1) {
		...
		// 10. 결과를 동영상 파일로 저장. 캡처하여 사진 저장
		writer << img_result;
		if (cnt++ == 15)
			imwrite("img_result.jpg", img_result); // 캡처하여 사진 저장
